...
def main(options,args) :
    ...
    if options.signal :
        ...
        for bin in range(3,h.GetNbinsX()+1) :
            text += ('%s:'%(h.GetXaxis().GetBinLabel(bin))).ljust(40)
            text += ' & '
            text += ('%2.2f \\pm %2.2f'%(h.GetBinContent(bin),h.GetBinError(bin))).rjust(17)
            text += ' & '
            text += ('%2.1f'%(100*h.GetBinContent(bin)/float(h.GetBinContent(max(bin-1,3))))).rjust(16)
            text += ' & '
            text += ('%2.1f'%(100*h.GetBinContent(bin)/float(h.GetBinContent(3)))).rjust(16)
            text += ' \\\\\n'
